...
<script>
	$(function(){
		...
		$("#frm").validate({
			...
			messages:{
				id:		{
					...
					minlength:"아이디는 3자 이상 입력하셔야 합니다",
					maxlength:"아이디는 8자 이상 입력하셔야 합니다"
					},
				pwd:	{
		...
		$(":button:first").click(function(){
			...
			window.open('<c:url value="/Back/IdCheck.do?id='+id+'"/>');
		});
                        ...
                        <form id="frm" action="<c:url value='/Back/Register.do'/>" method="post">
                            <fieldset>
                                <div class="form-group">
                                    <input class="form-control" placeholder="아이디는 영문 및 숫자만 가능합니다" id="idInput" name="id" type="txt" autofocus style="display: inline-block;width: 78%">
                                    <input type="button" class="btn btn-warning" value="id검사" style="display:inline; width: 18%"><br/>
                                    <label class="error" for="idInput" generated="true"></label>
                                </div>
                                <div class="form-group">
